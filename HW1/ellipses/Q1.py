...
if __name__ == "__main__":
    ...
    # 4-------------------------------------------- box image -------------------------------------------- #
    box = cv2.imread('nEKGD2wNiwqrTOc63kiWZT7b4.png')
    box_temp = cv2.resize(box, (box.shape[0] // 3, box.shape[1] // 3), interpolation=cv2.INTER_CUBIC)

    edge_map = cv2.Canny(box_temp, 100, 500, apertureSize=3, L2gradient=True)
    ...
    # 5-------------------------------------------- round image -------------------------------------------- #
    round_im = cv2.imread('nEKGD2wNiwqrTOc63kiWZT7b4.png')
    round_temp = cv2.resize(round_im, (round_im.shape[0] // 3, round_im.shape[1] // 3), interpolation=cv2.INTER_CUBIC)

    edge_map = cv2.Canny(round_temp, 100, 500, apertureSize=3, L2gradient=True)
    ...
    # 6-------------------------------------------- london image -------------------------------------------- #
    london = cv2.imread('nEKGD2wNiwqrTOc63kiWZT7b4.png')
    london_temp = cv2.resize(london, (london.shape[0] // 3, london.shape[1] // 3), interpolation=cv2.INTER_CUBIC)

    edge_map = cv2.Canny(london_temp, 100, 500, apertureSize=3, L2gradient=True)
