...
def match_corner(cor, window_ext=5):
    r, c = np.round(cor).astype(np.intp)
...
if __name__ == "__main__":
    ...
    img = cv2.imread('cameleon__N_8__sig_noise_5__sig_motion_103/source_01.jpg')
    img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    # generate synthetic checkerboard image and add gradient for the later matching
    checkerboard = img_as_float(img)
    img_orig = np.zeros(list(img.shape) + [3])
    img_orig[..., 0] = img
